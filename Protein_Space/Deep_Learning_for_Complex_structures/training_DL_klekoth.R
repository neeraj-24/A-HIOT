...
hyper_params <- list(hidden=list(c(1200,400),c(300,200,2)), input_dropout_ratio=c(0,0.05), rate=c(0.01,0.02), rate_annealing=c(1e-10,1e-9,1e-8))

grid <- h2o.grid(algorithm="deeplearning", grid_id="my_dl_grid", training_frame=train, validation_frame = test, x=predictors, y=y, epochs=50, stopping_metric="misclassification", reproducible=TRUE, stopping_tolerance=1e-2, stopping_rounds=2, adaptive_rate=F,  momentum_start=0.5,  momentum_stable=0.9, activation=c("Tanh"), max_w2=10, hyper_params=hyper_params)
